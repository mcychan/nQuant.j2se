...
public class BitmapUtilities {
	...
	protected static short[] quantize_image(final int width, final int height, final Color[] pixels, final Color[] palette, final Ditherable ditherable, final boolean hasSemiTransparency, final boolean dither)
	{
			...
			for (int i = 0; i < height; ++i) {
				...
				int a_pix = ditherPixel[3];
					...
					if(noBias) {
						...
						if (lookup[offset] == 0)
							lookup[offset] = (c.getAlpha() == 0) ? 1 : ditherable.nearestColorIndex(palette, c1) + 1;
						qPixels[pixelIndex] = (short) (lookup[offset] - 1);
					}
					else
						qPixels[pixelIndex] = (c.getAlpha() == 0) ? 0 : ditherable.nearestColorIndex(palette, c1);
		...
		for (int i = 0; i < qPixels.length; ++i)
			qPixels[i] = ditherable.nearestColorIndex(palette, pixels[i]);
