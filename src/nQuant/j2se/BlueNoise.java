...
public class BlueNoise {
	...
	public static Color diffuse(final Color pixel, final Color qPixel, final float weight, final float strength, final int x, final int y)
	{
		...
		r_pix = (int) Math.min(0xFF, Math.max(r_pix + (adj * (r_pix - qPixel.getRed())), 0.0));
		g_pix = (int) Math.min(0xFF, Math.max(g_pix + (adj * (g_pix - qPixel.getGreen())), 0.0));
		b_pix = (int) Math.min(0xFF, Math.max(b_pix + (adj * (b_pix - qPixel.getBlue())), 0.0));
		a_pix = (int) Math.min(0xFF, Math.max(a_pix + (adj * (a_pix - qPixel.getAlpha())), 0.0));                
		
		return new Color(r_pix, g_pix, b_pix, a_pix);
	}
	...
	public static short[] dither(final int width, final int height, final int[] pixels, final Color[] palette, final Ditherable ditherable, final short[] qPixels, final float weight)
	{	
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				...
				qPixels[bidx] = ditherable.nearestColorIndex(palette, c1, bidx);
			}
		}
